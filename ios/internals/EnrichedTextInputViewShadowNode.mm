...
Size EnrichedTextInputViewShadowNode::measureContent(
  ...
  if (weakWrapper != nullptr) {
    ...
    // on the very first call there is no componentView that we can query for
    // the component height thus, a little heuristic: just put a height that is
    // exactly height of letter "I" with default apple font and size from props
    ...
    NSAttributedString *attrStr = [[NSAttributedString alloc]
        initWithString:@"I"
            attributes:@{
              NSFontAttributeName :
                  [UIFont systemFontOfSize:typedProps.fontSize]
            }];
    ...
    const CGSize &suggestedSize = CTFramesetterSuggestFrameSizeWithConstraints(
        framesetter, CFRangeMake(0, 1), nullptr,
        CGSizeMake(layoutConstraints.maximumSize.width, DBL_MAX), nullptr);
