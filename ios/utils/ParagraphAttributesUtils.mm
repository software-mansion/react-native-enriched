...
// works especially bad when a list point just appears hence the solution -
...
+ (BOOL)handleBackspaceInRange:(NSRange)range
  ...
  // if the backspace removes the whole content of a paragraph (possibly more
  // but has to start where the paragraph starts), we remove the typing
  // attributes
  if (range.location == nonNewlineRange.location &&
      range.length >= nonNewlineRange.length) {
    // for lists and quotes we want to remove the characters but keep attribtues
    // so that a zero width space appears here so we do the removing manually
    // and reapply attributes
