    ...
    if (emitter != nullptr) {
      emitter->onChangeState({
        ...
        .isCodeBlock = NO, // [_activeStyles containsObject: @([CodeBlockStyle
                           // getStyleType])],
            .isImage = NO  // [_activeStyles containsObject: @([ImageStyle
                           // getStyleType]])],
      });
    }
  }
...
- (BOOL)handleStyleBlocksAndConflicts:(StyleType)type range:(NSRange)range {
  ...
  NSArray<NSNumber *> *blocking =
      [self getPresentStyleTypesFrom:_blockingStyles[@(type)] range:range];
  if (blocking.count != 0) {
    return NO;
  }
  ...
  NSArray<NSNumber *> *conflicting =
      [self getPresentStyleTypesFrom:_conflictingStyles[@(type)] range:range];
...
- (void)_performRelayout {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [textView.layoutManager invalidateLayoutForCharacterRange:wholeRange
