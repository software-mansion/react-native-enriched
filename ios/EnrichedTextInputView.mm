...
- (void)handleCommand:(const NSString *)commandName args:(const NSArray *)args {
  if ([commandName isEqualToString:@"focus"]) {
    ...
    [self applyAlignmentFromString:alignmentString];
  }
}
...
- (NSRange)expandRangeToContiguousList:(NSRange)range {
  ...
  NSArray *listStyles = @[
    stylesDict[@([UnorderedListStyle getStyleType])],
    stylesDict[@([OrderedListStyle getStyleType])],
    stylesDict[@([CheckboxListStyle getStyleType])]
  ];
