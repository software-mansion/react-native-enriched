...
- (NSString *)parseToHtmlFromRange:(NSRange)range {
  ...
  for (int i = 0; i < text.length; i++) {
    ...
    if ([[NSCharacterSet newlineCharacterSet]
      ...
      // get styles that have begun: they are sorted in a ascending manner to
      // properly keep tags' FILO order
...
- (NSArray *)getTextAndStylesFromHtml:(NSString *)fixedHtml {
  ...
  for (int i = 0; i < fixedHtml.length; i++) {
    ...
    if (currentCharacterChar == '<') {
      ...
      if ([currentTagName isEqualToString:@"p"] ||
        ...
        NSRange tagRange =
            NSMakeRange(tagLocation, plainText.length - tagLocation);
  ...
  for (NSArray *arr in initiallyProcessedTags) {
    ...
    if ([tagName isEqualToString:@"b"]) {
      ...
      NSString *url =
          [params substringWithRange:NSMakeRange(6, params.length - 7)];
